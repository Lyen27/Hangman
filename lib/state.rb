...
class State
  ...
  def render_state
    blank_lines = Array.new(4,"\n")
    display = [rope,blank_lines,mistakes,secret_word.join(' ')]
    puts display
  end

  def shift_drawing(str)
    space = "      "
    shifted_str = str.split("\n")
    .map {|row| space + row}
    .join("\n")
  end
  ...
  def get_random_word
    ...
    themes[theme].delete(random_word)
  end
  ...
  def check_guess(guess)
    if word.include?(guess)
      ...
      @rope = "=========\n" 
      @rope = rope + shift_drawing(hang_man.shift)
